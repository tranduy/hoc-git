import type { NativeMarketObservation, ProviderEvent, ProviderMarket, ProviderQuote } from "@tool-chenh/contracts";
import { isSupportedFootballTwoWayLine } from "../football-market-policy.js";
import { isSabaMultiMatchAggregate } from "./saba-multi-match-aggregate.js";

export interface SabaFootballNormalizeOptions {
  readonly observedAtMs: number;
  readonly receivedMonotonicMs: number;
  readonly sequence: number;
}

export interface NormalizedSabaFootballCatalog {
  readonly events: readonly ProviderEvent[];
  readonly markets: readonly ProviderMarket[];
  readonly quotes: readonly ProviderQuote[];
  readonly nativeMarketObservations: readonly NativeMarketObservation[];
  readonly diagnostics: readonly string[];
}

type RawRecord = Readonly<Record<string, unknown>>;
type SabaEventFamily = "GOALS" | "CORNERS" | "CARDS";

interface ClassifiedSabaEvent {
  readonly competition: string;
  readonly participants: readonly [string, string];
  readonly family: SabaEventFamily;
}

function id(value: unknown): string | null {
  if ((typeof value !== "string" && typeof value !== "number") || String(value).trim().length === 0) return null;
  return String(value).trim();
}

function text(value: unknown): string | null {
  return typeof value === "string" && value.trim().length > 0 ? value.trim() : null;
}

function finite(value: unknown): number | null {
  return typeof value === "number" && Number.isFinite(value) ? value : null;
}

function malay(value: unknown): string | null {
  const numeric = finite(value);
  if (numeric === null || numeric === 0 || Math.abs(numeric) > 1) return null;
  const normalized = String(numeric);
  return /^-?(?:0|[1-9]\d*)(?:\.\d+)?$/u.test(normalized) ? normalized : null;
}

function decimal(value: unknown): string | null {
  const numeric = finite(value);
  if (numeric === null || numeric <= 1) return null;
  const normalized = String(numeric);
  return /^(?:[1-9]\d*)(?:\.\d+)?$/u.test(normalized) ? normalized : null;
}

function isVirtualFootball(competition: string, participants: readonly string[]): boolean {
  const evidence = `${competition} ${participants.join(" ")}`.normalize("NFKC").toLocaleLowerCase("en");
  return /(?:soccer marble|e[\s-]?soccer|\bvirtual\b|simulated reality|spinner world cup|\bpes\b|\(pg\)|\(et\))/u.test(evidence);
}

function classifySabaEvent(competition: string, participants: readonly [string, string]): ClassifiedSabaEvent | null {
  const cornerCompetition = /\s*-\s*CORNERS\s*$/iu.test(competition);
  const bookingCompetition = /\s*-\s*BOOKINGS?\s*$/iu.test(competition);
  const cornerSuffix = /\s*(?:\(\s*)?No\.?\s*of\s+Corners(?:\s*\))?\s*$/iu;
  const bookingSuffix = /\s*(?:\(\s*)?Total\s+Bookings?(?:\s*\))?\s*$/iu;
  if (cornerCompetition || bookingCompetition) {
    const suffix = cornerCompetition ? cornerSuffix : bookingSuffix;
    if (!participants.every((participant) => suffix.test(participant))) return null;
    const normalized = participants.map((participant) => participant.replace(suffix, "").trim());
    if (normalized.some((participant) => participant.length === 0) || normalized[0] === normalized[1]) return null;
    // The suffix is what separates this book from the main match. Stripping it
    // left the corner and card books carrying the fixture's own competition, so
    // one fixture appeared three times under one name; a rival's price then
    // matched all three, could not say which it belonged to, and the main match
    // - the only entry it can actually be compared against - was withheld too.
    return {
      competition: competition.trim(),
      participants: normalized as [string, string],
      family: cornerCompetition ? "CORNERS" : "CARDS"
    };
  }
  if (participants.some((participant) => /\b(?:Corners?|Bookings?|Cards?)\s*$/iu.test(participant))) return null;
  return { competition, participants, family: "GOALS" };
}

function marketSemantics(family: SabaEventFamily, betType: number): {
  readonly marketType: "FT_AH" | "FT_TOTAL" | "FH_AH" | "FH_TOTAL" |
    "CORNER_FT_AH" | "CORNER_FT_TOTAL" | "CORNER_FH_AH" | "CORNER_FH_TOTAL" |
    "CARD_FT_AH" | "CARD_FT_TOTAL" | "CARD_FH_AH" | "CARD_FH_TOTAL" |
    "FT_ODD_EVEN" | "CORNER_FT_ODD_EVEN";
  readonly isHandicap: boolean;
  readonly isFirstHalf: boolean;
  readonly isLineFree: boolean;
  readonly settlementProfile: string;
} | null {
  if (betType === 2 && family !== "CARDS") {
    return {
      marketType: family === "CORNERS" ? "CORNER_FT_ODD_EVEN" : "FT_ODD_EVEN",
      isHandicap: false,
      isFirstHalf: false,
      isLineFree: true,
      settlementProfile: family === "CORNERS"
        ? "football-corners-odd-even-regulation" : "football-goals-odd-even-regulation"
    };
  }
  if (![1, 3, 7, 8].includes(betType)) return null;
  const isHandicap = betType === 1 || betType === 7;
  const isFirstHalf = betType === 7 || betType === 8;
  const prefix = family === "GOALS" ? "" : family === "CORNERS" ? "CORNER_" : "CARD_";
  const marketType = `${prefix}${isFirstHalf ? "FH" : "FT"}_${isHandicap ? "AH" : "TOTAL"}` as
    "FT_AH" | "FT_TOTAL" | "FH_AH" | "FH_TOTAL" |
    "CORNER_FT_AH" | "CORNER_FT_TOTAL" | "CORNER_FH_AH" | "CORNER_FH_TOTAL" |
    "CARD_FT_AH" | "CARD_FT_TOTAL" | "CARD_FH_AH" | "CARD_FH_TOTAL";
  const settlementProfile = family === "CORNERS"
    ? isFirstHalf ? "football-corners-first-half" : "football-corners-regulation"
    : family === "CARDS"
      ? isFirstHalf ? "football-cards-first-half" : "football-cards-regulation"
      : isFirstHalf ? "football-first-half-including-added-time" : "football-regulation-including-added-time";
  return { marketType, isHandicap, isFirstHalf, isLineFree: false, settlementProfile };
}

function canonicalHomeHandicap(record: RawRecord): string | null {
  const homeMagnitude = finite(record.hdp1);
  const awayMagnitude = finite(record.hdp2);
  if (homeMagnitude === null || awayMagnitude === null || homeMagnitude < 0 || awayMagnitude < 0 ||
    (homeMagnitude > 0 && awayMagnitude > 0)) return null;
  const value = awayMagnitude - homeMagnitude;
  if (!Number.isFinite(value)) return null;
  const canonical = String(value);
  return isSupportedFootballTwoWayLine(canonical) ? canonical : null;
}

function canonicalTotalLine(record: RawRecord): string | null {
  const total = finite(record.hdp1);
  const secondary = finite(record.hdp2);
  if (total === null || secondary !== 0 || total <= 0) return null;
  const canonical = String(total);
  return isSupportedFootballTwoWayLine(canonical) ? canonical : null;
}

/** A kickoff no further from the observation than this restates it rather than
 * dating the fixture, so it is not evidence of when the match starts. */
const SABA_KICKOFF_OBSERVATION_MARGIN_MS = 60_000;

/** A kickoff must sit this far ahead before it can contradict a live claim, so a
 * fixture seconds from kick-off is not read as one that starts later. */
const SABA_SCHEDULED_KICKOFF_MARGIN_MS = 300_000;

function participantPairKey(home: string, away: string): string {
  return [home.trim().toLowerCase(), away.trim().toLowerCase()].sort().join("\u0000");
}

function informativeKickoff(value: unknown, observedAtMs: number): boolean {
  const seconds = finite(value);
  return seconds !== null && seconds >= 0 &&
    Math.abs(seconds * 1_000 - observedAtMs) > SABA_KICKOFF_OBSERVATION_MARGIN_MS;
}

export function normalizeSabaFootballRecords(
  records: readonly RawRecord[],
  options: SabaFootballNormalizeOptions
): NormalizedSabaFootballCatalog {
  const events: ProviderEvent[] = [];
  const markets: ProviderMarket[] = [];
  const quotes: ProviderQuote[] = [];
  const nativeMarketObservations: NativeMarketObservation[] = [];
  const diagnostics: string[] = [];
  if (!Number.isFinite(options.observedAtMs) || !Number.isFinite(options.receivedMonotonicMs) ||
    !Number.isSafeInteger(options.sequence) || options.sequence < 0) {
    return { events, markets, quotes, nativeMarketObservations,
      diagnostics: ["SABA_FOOTBALL_OPTIONS_INVALID"] };
  }

  const leagues = new Map<string, string>();
  for (const record of records) {
    if (record.type !== "l") continue;
    const leagueId = id(record.leagueid);
    const name = text(record.leaguenameen);
    if (leagueId !== null && name !== null && (record.sporttype === undefined || record.sporttype === 1)) {
      leagues.set(leagueId, name);
    }
  }

  // SABA repeats a fixture across its market groups, and the live group stamps
  // kickofftime with the moment the snapshot was taken rather than the kickoff.
  // Measured 2026-08-27: 59 of 80 events carried a kickoff within a minute of
  // the observation, while the books that do publish one placed the same
  // fixtures a median of twelve hours away. Letting that value overwrite a real
  // kickoff erases the evidence that decides both the phase and the pre-match
  // pairing window, so an upcoming fixture claims to be live and can never be
  // compared against another book's pre-match one.
  const matches = new Map<string, RawRecord>();
  for (const record of records) {
    if (record.type !== "m") continue;
    const matchId = id(record.matchid);
    if (matchId === null) continue;
    const previous = matches.get(matchId);
    const keepPreviousKickoff = previous !== undefined &&
      !informativeKickoff(record.kickofftime, options.observedAtMs) &&
      informativeKickoff(previous.kickofftime, options.observedAtMs);
    matches.set(matchId, { ...(previous ?? {}), ...record,
      ...(keepPreviousKickoff ? { kickofftime: previous.kickofftime } : {}) });
  }

  // SABA's live market group carries the main-match odds for fixtures that have
  // not kicked off, stamped with the moment the snapshot was built rather than a
  // kickoff: measured 2026-08-27, 63 of its 66 live events shared one timestamp
  // to the second, while the same fixtures appear again in the day's list with a
  // real, whole-minute kickoff hours away. SABA's own snapshot contradicts the
  // live claim, and a fixture that starts later is not running now. Believing
  // the claim strands the only markets worth comparing - 47 CMD, 18 BTI and 11
  // IM pairings at the time of measurement - because a live event is never
  // compared against another book's pre-match one.
  const scheduledKickoffByPair = new Map<string, number>();
  for (const record of matches.values()) {
    const seconds = finite(record.kickofftime);
    const home = text(record.hteamnameen);
    const away = text(record.ateamnameen);
    if (seconds === null || home === null || away === null ||
      seconds * 1_000 <= options.observedAtMs + SABA_SCHEDULED_KICKOFF_MARGIN_MS) continue;
    const pair = participantPairKey(home, away);
    const previous = scheduledKickoffByPair.get(pair);
    if (previous === undefined || seconds < previous) scheduledKickoffByPair.set(pair, seconds);
  }

  const acceptedMatches = new Set<string>();
  const classifiedMatches = new Map<string, ClassifiedSabaEvent>();
  const liveByMatchId = new Map<string, boolean>();
  for (const [matchId, match] of matches) {
    const leagueId = id(match.leagueid);
    const participantA = text(match.hteamnameen);
    const participantB = text(match.ateamnameen);
    const kickoffSeconds = finite(match.kickofftime);
    const competition = leagueId === null ? null : leagues.get(leagueId) ?? text(match.leaguenameen);
    if (match.sporttype !== 1 || leagueId === null || participantA === null || participantB === null ||
      participantA === participantB || competition === null || kickoffSeconds === null || kickoffSeconds < 0) {
      diagnostics.push("SABA_FOOTBALL_EVENT_REJECTED");
      continue;
    }
    const classified = classifySabaEvent(competition, [participantA, participantB]);
    const isVirtual = isVirtualFootball(competition, [participantA, participantB]);
    // ET/PEN are separately settled derivative events on SABA, not the same
    // regulation-time ticket. Virtual events are outside this Football feed.
    if (isVirtual || classified === null || participantsHaveDerivativeSuffix([participantA, participantB]) ||
      isSabaMultiMatchAggregate(competition, [participantA, participantB])) {
      diagnostics.push("SABA_FOOTBALL_EVENT_UNSUPPORTED");
      continue;
    }
    // SABA's live market group also carries fixtures that have not kicked off,
    // with no period, clock or score to contradict it. Its own kickoff time is
    // the evidence that decides the phase; a mislabelled live event can never
    // be compared against another book's pre-match one.
    const scheduledSeconds = scheduledKickoffByPair.get(participantPairKey(participantA, participantB));
    const startsLater = scheduledSeconds !== undefined && kickoffSeconds * 1_000 <= options.observedAtMs;
    const startAtSeconds = startsLater ? scheduledSeconds : kickoffSeconds;
    const isLive = match.marketid === "L" && !startsLater && startAtSeconds * 1_000 <= options.observedAtMs;
    events.push({
      provider: "SABA", category: "FOOTBALL", providerEventId: matchId, competition: classified.competition,
      seasonStage: null, startAtUtcMs: startAtSeconds * 1_000,
      participantA: classified.participants[0], participantB: classified.participants[1],
      eventScope: "REGULATION", bestOf: null, isLive, rematchCandidate: isLive,
      fixtureDiscriminator: null, isVirtual: false, sportVariant: "FOOTBALL",
      liveState: isLive ? { period: null, scoreHome: null, scoreAway: null, clockMs: null } : null
    });
    acceptedMatches.add(matchId);
    liveByMatchId.set(matchId, isLive);
    classifiedMatches.set(matchId, classified);
  }

  const seenMarkets = new Set<string>();
  for (const [recordIndex, record] of records.entries()) {
    if (record.type !== "o") continue;
    const matchId = id(record.matchid);
    const oddsId = id(record.oddsid);
    const nativeType = id(record.bettype) ?? "UNKNOWN";
    const providerEventId = matchId ?? `invalid-event:${recordIndex}`;
    const providerMarketId = oddsId ?? `${providerEventId}:native:${nativeType}:${recordIndex}`;
    const classified = matchId === null ? undefined : classifiedMatches.get(matchId);
    const semantics = classified === undefined || !Number.isSafeInteger(Number(record.bettype))
      ? null : marketSemantics(classified.family, Number(record.bettype));
    const isCleanSheet = classified?.family === "GOALS" && Number(record.bettype) === 13;
    const nativeScope = isCleanSheet ? "FULL_TIME"
      : semantics === null ? null : semantics.isFirstHalf ? "FIRST_HALF" : "FULL_TIME";
    const defaultOutcomes = isCleanSheet ? ["HOME_YES", "HOME_NO", "AWAY_YES", "AWAY_NO"]
      : semantics?.isHandicap === true ? ["HOME", "AWAY"]
      : semantics?.isLineFree === true ? ["ODD", "EVEN"]
        : semantics === null ? ["OUTCOME_1", "OUTCOME_2"] : ["OVER", "UNDER"];
    const observe = (disposition: NativeMarketObservation["disposition"], reason: string): void => {
      // Preserve provider field identities even when their outcome semantics are
      // unknown; OUTCOME_1/2 alone cannot be replayed into a canonical contract.
      const nativeSelections = Object.entries(record).flatMap(([field, value]) => {
        if (!/^(?:odds\d+a|cs\d+)$/u.test(field) || finite(value) === null) return [];
        const rawLine = field === "odds1a" ? finite(record.hdp1)
          : field === "odds2a" ? finite(record.hdp2) : null;
        return [{ selectionId: null, outcomeId: field, line: rawLine === null ? null : String(rawLine),
          price: String(value), ...(isCleanSheet ? { rawFormat: "DECIMAL" as const }
            : semantics !== null ? { rawFormat: "MALAY" as const } : {}) }];
      });
      nativeMarketObservations.push({
        provider: "SABA", category: "FOOTBALL", providerEventId, providerMarketId,
        nativeType, nativeLabel: null, nativeScope, outcomeLabels: defaultOutcomes,
        observedAtMs: options.observedAtMs, disposition, reason, nativeSelections
      });
    };
    if (matchId === null || !acceptedMatches.has(matchId)) {
      observe("EXCLUDED", "EVENT_NOT_COMPARABLE");
      continue;
    }
    if (record.parenttypeid !== undefined && id(record.parenttypeid) !== nativeType) {
      observe("EXCLUDED", "PARENT_MARKET_VARIANT");
      continue;
    }
    if (isCleanSheet) {
      const homeYes = decimal(record.cs11);
      const homeNo = decimal(record.cs10);
      const awayYes = decimal(record.cs21);
      const awayNo = decimal(record.cs20);
      if (oddsId === null || [homeYes, homeNo, awayYes, awayNo].some((price) => price === null) ||
        seenMarkets.has(providerMarketId)) {
        observe("EXCLUDED", seenMarkets.has(providerMarketId)
          ? "DUPLICATE_NATIVE_MARKET_ID" : "INVALID_TWO_WAY_SHAPE");
        diagnostics.push("SABA_FOOTBALL_MARKET_REJECTED");
        continue;
      }
      seenMarkets.add(oddsId);
      const status = record.oddsstatus === "running" && record.enable !== 0 ? "OPEN" as const : "SUSPENDED" as const;
      const isLive = liveByMatchId.get(matchId) === true;
      const cleanSheetMarkets = [
        { suffix: "home-clean-sheet", marketType: "HOME_FT_CLEAN_SHEET",
          settlementProfile: "football-home-clean-sheet", prices: [["YES", homeYes!], ["NO", homeNo!]] },
        { suffix: "away-clean-sheet", marketType: "AWAY_FT_CLEAN_SHEET",
          settlementProfile: "football-away-clean-sheet", prices: [["YES", awayYes!], ["NO", awayNo!]] }
      ] as const;
      for (const cleanSheet of cleanSheetMarkets) {
        const marketId = `${oddsId}:${cleanSheet.suffix}`;
        markets.push({
          provider: "SABA", category: "FOOTBALL", providerEventId: matchId, providerMarketId: marketId,
          marketType: cleanSheet.marketType, scope: "FULL_TIME", line: null,
          settlementProfile: cleanSheet.settlementProfile, status
        });
        quotes.push(...cleanSheet.prices.map(([selection, rawOdds]): ProviderQuote => ({
          provider: "SABA", category: "FOOTBALL", providerEventId: matchId,
          providerMarketId: marketId, providerSelectionId: `${marketId}:${selection.toLowerCase()}`,
          marketType: cleanSheet.marketType, scope: "FULL_TIME", selection, line: null,
          rawOdds, rawFormat: "DECIMAL", status, isLive, sourceTimestampMs: null,
          receivedMonotonicMs: options.receivedMonotonicMs, sequence: options.sequence
        })));
      }
      observe("NORMALIZED", "CANONICAL_MARKET_MAPPED");
      continue;
    }
    if (semantics === null) {
      observe("UNMAPPED", "NATIVE_TYPE_UNMAPPED");
      continue;
    }
    const { marketType, isHandicap, isFirstHalf, isLineFree, settlementProfile } = semantics;
    const scope = isFirstHalf ? "FIRST_HALF" as const : "FULL_TIME" as const;
    const line = isLineFree ? null : isHandicap ? canonicalHomeHandicap(record) : canonicalTotalLine(record);
    const firstPrice = malay(record.odds1a);
    const secondPrice = malay(record.odds2a);
    if (oddsId === null || (!isLineFree && line === null) || firstPrice === null || secondPrice === null ||
      seenMarkets.has(oddsId)) {
      observe("EXCLUDED", seenMarkets.has(providerMarketId)
        ? "DUPLICATE_NATIVE_MARKET_ID" : "INVALID_TWO_WAY_SHAPE");
      diagnostics.push("SABA_FOOTBALL_MARKET_REJECTED");
      continue;
    }
    seenMarkets.add(oddsId);
    const status = record.oddsstatus === "running" && record.enable !== 0 ? "OPEN" as const : "SUSPENDED" as const;
    const isLive = liveByMatchId.get(matchId) === true;
    markets.push({
      provider: "SABA", category: "FOOTBALL", providerEventId: matchId, providerMarketId: oddsId,
      marketType, scope, line,
      settlementProfile, status
    });
    const selections = isHandicap ? ["HOME", "AWAY"] as const
      : isLineFree ? ["ODD", "EVEN"] as const : ["OVER", "UNDER"] as const;
    quotes.push(...selections.map((selection, index): ProviderQuote => ({
      provider: "SABA", category: "FOOTBALL", providerEventId: matchId, providerMarketId: oddsId,
      providerSelectionId: `${oddsId}:${selection.toLowerCase()}`, marketType, scope,
      selection, line, rawOdds: index === 0 ? firstPrice : secondPrice, rawFormat: "MALAY", status, isLive,
      sourceTimestampMs: null, receivedMonotonicMs: options.receivedMonotonicMs, sequence: options.sequence
    })));
    observe("NORMALIZED", "CANONICAL_MARKET_MAPPED");
  }

  return { events, markets, quotes, nativeMarketObservations, diagnostics };
}

function participantsHaveDerivativeSuffix(participants: readonly string[]): boolean {
  return participants.some((participant) => /\((?:ET|PEN)\)\s*$/iu.test(participant));
}
