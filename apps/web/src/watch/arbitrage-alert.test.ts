...
describe("watched arbitrage alert planning", () => {
  it("builds an exact rounded two-outcome plan from two open books", () => {
    ...
    expect(alert?.fingerprint).toContain("SABA|OVER|SABA-event|SABA-FT_TOTAL|SABA-OVER|1.8|100000");
    expect(alert?.fingerprint).toContain("SBOBET|UNDER|SBOBET-event|SBOBET-FT_TOTAL|SBOBET-UNDER|2.5|72000");
  });
