  ...
  return (
    <li className="opportunity-leg">
      ...
      <dl>
        ...
        <div><dt>Financial policy</dt><dd><span aria-label={financialPolicyLabel} title={`FX as of ${leg.fxAsOfMs}`}>{leg.feeRate === null ? "No fee" : `${leg.feeType} ${formatPercent(leg.feeRate)}`} · {leg.stakeCurrency}→{leg.baseCurrency} @ {leg.fxRate} · spread {formatPercent(leg.fxSpreadRate)}</span></dd></div>
