...
const snapshot: AppSnapshot = {
  ...
  events: [
    ...
    { canonicalEventId: "event-review", category: "LOL", competition: "Summer Split", seasonStage: "Playoffs", startAtUtcMs: 2, participantA: "Blue", participantB: "Red", providerEventIds: ["c", "d"], mappingStatus: "REVIEW_REQUIRED", mappingEvidence: [evidence("same tournament", false)] },
...
describe("MappingsPage", () => {
  ...
  it("filters every mapping status and expands expected, actual, pass/fail, and reason evidence without write controls", () => {
    render(<MappingsPage snapshot={snapshot} />);
    ...
    expect(screen.getByText("same tournament expected")).toBeTruthy();
    expect(screen.getByText("same tournament actual")).toBeTruthy();
  ...
  it("gives an empty filter result a safe read-only next action", () => {
    render(<MappingsPage snapshot={{ ...snapshot, events: [], markets: [] }} />);
