...
export function MappingsPage({ snapshot, connectionState }: { readonly snapshot: AppSnapshot; readonly connectionState: ConnectionState }) {
  ...
  return (
    <>
      ...
      {connectionState !== "LIVE" ? <section className="empty-state connection-warning" role="alert"><h2>{connectionState === "DISCONNECTED" ? "Connection disconnected" : "Validating live connection"}</h2><p>Mapping evidence is last-known and non-actionable until a validated fresh server snapshot returns. Reconnect to the local feed and wait for a fresh server snapshot.</p></section> : null}
      {snapshot.blockedDiagnostics.filter(isStaleDiagnostic).map((diagnostic) => <p className="stale-warning" role="status" key={`${diagnostic.category}:${diagnostic.canonicalMarketId ?? "category"}:${diagnostic.reason}`}>Stale server evidence: {diagnostic.reason}. Wait for a fresh server snapshot.</p>)}
