...
export class ProviderPreflightRegistry {
  ...
  async preflight(input: ProviderTicketPreflightRequest): Promise<ProviderTicketPreflight> {
    ...
    const account = (await this.#accounts.listStatuses()).find((candidate) => candidate.id === request.accountId);
    ...
    const result = await this.#accounts.withActiveHandle(account.id, account.provider,
      async (handle) => reader.preflight(handle, request));
    const parsed = ProviderTicketPreflightSchema.parse(result);
    if (parsed.accountId !== request.accountId || parsed.provider !== account.provider ||
