...
describe("ExecutionSafetyGate", () => {
  ...
  it("requires an exact confirmation and consumes a short-lived token once", () => {
    ...
    const gate = new ExecutionSafetyGate({ enabled: true, clock: { nowMs: () => now },
      ttlMs: 1000, readinessCheck: () => true, tokenFactory: () => "arm-token-123456" });
  ...
  it("latches the kill switch and invalidates every outstanding arm", () => {
    const gate = new ExecutionSafetyGate({ enabled: true, clock: { nowMs: () => 1000 },
      readinessCheck: () => true, tokenFactory: () => "arm-token-123456" });
    const token = gate.arm("ticket-1", "ARM ticket-1");
  ...
  it("cannot be enabled or armed without a current adapter and receipt readiness check", () => {
    expect(() => new ExecutionSafetyGate({ enabled: true })).toThrow("LIVE_READINESS_CHECK_REQUIRED");
    let ready = false;
    const gate = new ExecutionSafetyGate({ enabled: true, readinessCheck: () => ready,
      tokenFactory: () => "arm-token-123456" });
    expect(() => gate.arm("ticket-1", "ARM ticket-1")).toThrow("LIVE_EXECUTION_NOT_READY");
    ready = true;
    expect(gate.arm("ticket-1", "ARM ticket-1")).toBe("arm-token-123456");
  });
});
