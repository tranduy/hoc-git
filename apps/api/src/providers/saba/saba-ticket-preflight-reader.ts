  ...
  async preflight(handle: ActiveSecretHandle, request: ProviderTicketPreflightRequest): Promise<ProviderTicketPreflight> {
    ...
    return handle.withSecret(async (secret) => {
      ...
      return { accountId: request.accountId, provider: "SABA", providerEventId: quote.providerEventId,
        ...
        selection: quote.selection, line: quote.line, rawOdds, rawFormat: quote.rawFormat,
        decimalOdds, quoteStatus: quote.status, providerObservedAtMs: fresh ? limit.observedAtMs : observedAtMs,
        receivedMonotonicMs: quote.receivedMonotonicMs, sequence: quote.sequence, limitEvidence, constraint,
