...
export class SabaObservedCatalogReader {
  ...
  async read(accountId: string): Promise<ObservedProviderCatalog> {
    ...
    try {
      records = await this.#accounts.withActiveHandle(accountId, "SABA", async (handle) =>
        handle.withSecret(async (secret) => {
          if (secret.kind !== "LAUNCH_URL") throw new Error("SABA_FOOTBALL_CATALOG_UNAVAILABLE");
    ...
    if (normalized.events.length === 0 || normalized.markets.length === 0) {
      throw new Error("SABA_FOOTBALL_CATALOG_SCHEMA_ERROR");
    }
