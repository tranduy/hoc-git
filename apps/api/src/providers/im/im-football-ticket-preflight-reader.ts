...
import type { ImFootballJitCatalogSource } from "./im-football-observed-catalog.js";

function plain(value: ReturnType<typeof toDecimal>): string { return value.toFixed(value.decimalPlaces()); }

export class ImFootballTicketPreflightReader implements ProviderTicketPreflightReader {
  readonly provider = "IM" as const;
  readonly capabilities = ["PREFLIGHT"] as const;
  readonly #source: ImFootballJitCatalogSource;

  constructor(options: { readonly source: ImFootballJitCatalogSource }) { this.#source = options.source; }

  async preflight(handle: ActiveSecretHandle, request: ProviderTicketPreflightRequest): Promise<ProviderTicketPreflight> {
    if (handle.provider !== "IM" || handle.category !== "FOOTBALL") throw new Error("PREFLIGHT_ACCOUNT_UNAVAILABLE");
    return handle.withSecret(async (secret) => {
      ...
      const normalized = normalizeSbobetCatalog(snapshot.records, { provider: "IM",
      ...
      return { accountId: request.accountId, provider: "IM", providerEventId: quote.providerEventId,
