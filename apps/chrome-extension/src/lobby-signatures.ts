...
export function shouldPreserveSabaObserver(
  attachedLobby: ChromeLobbyId,
  tab: TabDescriptor
): boolean {
  if (attachedLobby !== "SABA" || !tab.url) return false;
  try {
    const url = new URL(tab.url);
    const hostname = url.hostname.toLowerCase();
    const ownedHost = /^c0z0o[a-z0-9]+\.bp[a-z0-9]+\.com$/iu.test(hostname) &&
      !/^c0z0o[a-z0-9]+\.(?:bpb7jrm5|bpf7t7s9)\.com$/iu.test(hostname);
    return ownedHost && !isSabaErrorUrl(url) &&
      !isSabaErrorTitle(tab.title?.trim() ?? "");
  } catch {
    return false;
  }
}
