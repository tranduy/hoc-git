Bạn phụ trách DUY NHẤT sàn SBOBET trong dự án tool-chenh. Đây là yêu cầu thực hiện, không chỉ lập kế hoạch. Mục tiêu là hoàn thành thu thập toàn bộ kèo bóng đá prematch sàn cung cấp, gồm kèo ẩn, đồng thời duy trì cập nhật giá thực theo nhịp 3 giây khi có thay đổi. Không chờ SABA hoàn thành toàn bộ nhiệm vụ mới làm phần độc lập.
...
2. Chỉ sửa trong checkout/snapshot riêng dành cho SBOBET, xuất phát từ baseline nhất quán đã công bố, gồm các sửa đổi chưa commit cần thiết. Không tự dùng HEAD cũ rồi coi đó là mã hiện tại; không stash/reset/checkout/commit hộ cây đang chạy. Không chuyển cuộc hội thoại SABA hoặc di chuyển thay đổi của nó.
...
PHẦN RIÊNG SBOBET
- Lobby KSPORT là SBOBET trong hệ thống này; account catalog-source:SBOBET:FOOTBALL. Xác minh mapping hiện tại trước kiểm tra live.
- Đọc cả direct/detail catalog và đường KSPORT/socket thực tế, không chỉ sửa sbobet-socketio-adapter vì tên tệp.
- Kiểm tra góc/thẻ và pseudo-event bị loại nhầm, liên kết trận gốc, nhóm ẩn và authoritative detail membership. Phân biệt derivative hợp lệ với virtual/eSoccer.
- Chứng minh socket cập nhật đúng selection/line trong detail kể cả khi chưa mở giao diện nhóm đó; nếu chưa có subscription thì điều tra subscription thay vì chỉ làm giá fresh trên giấy.
- Giữ các sửa đổi đang phục vụ APSPORT: packages/adapters/src/sbobet/sbobet-normalizer.ts là mã dùng chung thực tế, dù tên là sbobet. Mọi sửa đổi ở đó gửi integrator và yêu cầu regression AP.

PHẠM VI TỆP RIÊNG
- apps/api/src/providers/sbobet/sbobet-direct-catalog.ts
- apps/api/src/providers/sbobet/sbobet-observed-catalog.ts
- apps/api/src/providers/sbobet/sbobet-browser-manager.ts
- apps/api/src/chrome-bridge/ksport-ws-adapter.ts
- apps/api/src/chrome-bridge/sbobet-socketio-adapter.ts
- Test tương ứng, fixture đã loại dữ liệu nhạy cảm và báo cáo sbobet riêng trong checkout.
- Module collector/discovery/refresh mới có tiền tố sbobet- trong apps/chrome-extension/src nếu cần; tự hoàn thiện điểm nối ở tệp chung trong checkout riêng và xuất patch riêng để review/ghép.
