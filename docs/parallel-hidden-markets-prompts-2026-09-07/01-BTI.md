Bạn phụ trách DUY NHẤT sàn BTI trong dự án tool-chenh. Đây là yêu cầu thực hiện, không chỉ lập kế hoạch. Mục tiêu là hoàn thành thu thập toàn bộ kèo bóng đá prematch sàn cung cấp, gồm kèo ẩn, đồng thời duy trì cập nhật giá thực theo nhịp 3 giây khi có thay đổi. Không chờ SABA hoàn thành toàn bộ nhiệm vụ mới làm phần độc lập.
...
2. Chỉ sửa trong checkout/snapshot riêng dành cho BTI, xuất phát từ baseline nhất quán đã công bố, gồm các sửa đổi chưa commit cần thiết. Không tự dùng HEAD cũ rồi coi đó là mã hiện tại; không stash/reset/checkout/commit hộ cây đang chạy. Không chuyển cuộc hội thoại SABA hoặc di chuyển thay đổi của nó.
...
PHẦN RIÊNG BTI
- Lobby BTI; account catalog-source:BTI:FOOTBALL.
- Collector đang ở BTI_CATALOG_REFRESH_EXPRESSION trong network-observer.ts: hydrate league live/prematch, queue detail các event prematch. API đi qua bti-http-adapter.ts và bti-direct-catalog.ts. Đây là nền hiện có, chưa phải bằng chứng coverage/realtime hiện tại.
- Điều tra hết league/event, các nhóm detail và mã market ngoài nhóm chính; kiểm tra detail cache/eviction, queue pending/failure và đường giá cập nhật tiếp sau bootstrap. Không coi “pending = 0” là đủ khi cache hoặc parser có thể thiếu dữ liệu.
- Giữ quyền dữ liệu HTTP thực tế của BTI; tách roster/detail khỏi cập nhật giá và kiểm tra kèo góc/thẻ cùng các mã chưa map. Không mặc định browser-manager fallback chính là collector extension hiện hành.

PHẠM VI TỆP RIÊNG
- apps/api/src/providers/bti/bti-direct-catalog.ts
- apps/api/src/providers/bti/bti-observed-catalog.ts
- apps/api/src/providers/bti/bti-browser-manager.ts
- apps/api/src/chrome-bridge/bti-http-adapter.ts
- Test tương ứng, fixture đã loại dữ liệu nhạy cảm và báo cáo bti riêng trong checkout.
- Module collector/discovery/refresh mới có tiền tố bti- trong apps/chrome-extension/src nếu cần; tự hoàn thiện điểm nối ở tệp chung trong checkout riêng và xuất patch riêng để review/ghép.
