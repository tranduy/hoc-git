...
| 41 | BTI: mở rộng `early` hỏng bao nhiêu lần | Trước chỉ có `earlyExpansionRefused` mà không có mẫu số — `rosterStarts` tính từ lúc tab nạp, bộ đếm tính từ lúc build, chia nhau ra sẽ là số **giả**. Thêm `earlyExpansionRuns` đếm cùng cửa sổ. Đọc được: **0/28** — sự cố làm BTI tối 11 tiếng **chưa tái diễn lần nào**; `earlyExpandLeagues` giữ 192→200, dòng có tên giữ 120 | `catalogShape` |
...
|---|---|---|---|
| 43 | SABA: số trận công bố dao động 649 ↔ 131 | Ba ảnh chụp có mốc giờ: **37 → 649 trận / 3.366 market (+337s) → 131 (+916s)**, id phân biệt, cả ba `FRESH`; lấy mẫu 6 phút sau đó đứng yên **130–131**. Census của collector cho biết roster thật: `owners=t119.m106.d3, e20.m11.d5` → TODAY **119** trận, EARLY **20** — khớp với 131, **không khớp 649**. Nên 649 là ảnh chụp ngay sau restart API, chưa rõ nó gồm gì. **Chỉ là số trận, không liên quan kèo ẩn** (xem TRẦN) | Chưa làm |
