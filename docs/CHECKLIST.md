...
| 22 | Kèo góc có bị thu sót không | **Không.** SBOBET **0**, CMD **0** kèo góc thấy-mà-không-chuẩn-hoá-được | `nativeMarketObservations`. Đo lại 2026-09-16 qua đường từ chối theo từng trận: loại `NATIVE_TYPE_UNMAPPED` của CMD hoá ra là **clean sheet, không phải góc** (mục 39) — trần kèo góc vẫn đứng nguyên |
...
| 39 | **CMD `MORE:FT:3` — loại native chưa map lớn nhất hệ thống** | Đi tìm lý do thiếu kèo góc thì lộ ra: trong số các sàn được ghép vào trận CÓ kèo góc mà **không** chào góc, APSPORT/SBOBET chỉ từ chối `OTHER_SCORE_DOMAIN_REQUIRED` và IM chỉ `NON_BINARY_OUTCOME_DOMAIN` — **trần đã chứng minh**, không phải mất. CMD từ chối `NATIVE_TYPE_UNMAPPED` ở **26/26**. Loại đó là `MORE:FT:3`: **210 trận**, 4 giá một dòng, và CMD **không** chào clean sheet ở bất kỳ đâu khác trong khi SBOBET/APSPORT/BTI đều có. Thứ tự `[home YES, home NO, away YES, away NO]` **do sàn khác chốt, không suy từ hình dạng**: trên 37 trận chung, sai số giá trung vị **3,7%** (143/148 trong 10%); đọc ngược là **28,8%** (23/148). Thu được: **243 dòng clean sheet, 193 có chân CMD, 97 dòng chỉ tồn tại nhờ nó**, CMD tốt nhất ở **0** ô — thêm chiều sâu, chưa tạo chênh lệch | `cmd-more-categorical.test.ts` + `cleanSheetValue` |
| 40 | IM `EVENT_NOT_COMPARABLE` — tách 6 nguyên nhân | Nhóm từ chối **lớn nhất toàn hệ thống**: 26.865 market / **1.035 trận**, nhiều hơn cả danh mục IM công bố (757). Tách xong đọc lại: **`EVENT_VIRTUAL_FOOTBALL` 1.025 trận**, năm nhánh còn lại **bằng 0** — không mất trận nào, không thiếu tên, không thiếu cờ. **Trần, không phải lỗ hổng.** Cờ cyber *vắng* được tách riêng khỏi cờ *bật* (`EVENT_CYBER_FLAG_MISSING`): trường vắng không phải bằng chứng trận là ảo | `im-football-catalog-source.test.ts` |
| 41 | BTI: mở rộng `early` hỏng bao nhiêu lần | Trước chỉ có `earlyExpansionRefused` mà không có mẫu số — `rosterStarts` tính từ lúc tab nạp, bộ đếm tính từ lúc build, chia nhau ra là số **giả**. Thêm `earlyExpansionRuns` đếm cùng cửa sổ. Đo được: **1/336**. Và lần hỏng đó **đã được chứng kiến đang hoạt động đúng**: `requestStatus:503`, `lastRosterFailure:pages-failed-early`, mà catalog vẫn `phase:COMPLETE` với **1.693 trận** — trước bản vá hôm nay, đúng một lần từ chối đó sẽ xoá sạch BTI | `catalogShape` |
| 42 | SABA `EVENT_TIME_UNRESOLVED` — tách 3 nguyên nhân | **`EVENT_KICKOFF_DATE_UNKNOWN` 96 trận**, `EVENT_TIMEZONE_OFFSET_INVALID` **0**, `EVENT_TIME_TEXT_UNRECOGNISED` **0**. Tức là: có đồng hồ nhưng collector **không xác định được ngày**, nên từ chối thay vì đoán — **đúng nguyên tắc, không phải hỏng**. Khác e-soccer ở chỗ đây **sửa được**: ngày có trên trang, chỉ là chưa đưa vào capture | `cmd-normalizer.test.ts` |
| 45 | **Dashboard đơ: tìm ra và vá được 2 chỗ, có số trước/sau** | Anh báo scroll không nhúc nhích. **Bốn giả thuyết của tôi đều sai, mỗi cái bị một con số giết:** API dựng đúng request app gọi trong **46–73ms**; app tải **11,6 MB / 19 request** (không phải 204 MB — tôi đo view `nativeDetail=full` mà app **không bao giờ gọi**); `JSON.parse` **28ms**/90s; `postMessage` (clone sang worker) **31ms**; `JSON.stringify` 220ms; DOM **212 node**. `marketTypes=paired` thì **đã dùng sẵn** từ trước. Nguyên nhân thật: **dây chuyền xếp hạng chạy trên luồng chính mỗi vòng poll**. Vá 2 chỗ, đều là memo hoá **hàm thuần** (khoá đúng bằng đầu vào hàm đọc, không đổi kết quả): `oddsOf` và `comparableFootballProduct`. Đo: `decimal.js` **4,8% → 0,8%**, long task nặng nhất **2.009ms → 365ms**, tổng chặn **21.658 → 11.630ms**. Còn ~365ms/vòng, đỉnh kế tiếp là `quoteIdentity` | `apps/web` 1.166 test xanh |
| 46 | **API ăn 75–91% một lõi: gọi được tên, chưa vá** | Bật `--cpu-prof` (opt-in qua `FIELDLINE_API_CPU_PROF_DIR`, mặc định tắt) vì diag không đo thời gian hop nào. Profile 407s: **idle 54,8% · GC 7,2% · `catalog-revision-hasher` 4,5%+1,7% · `recordCatalog` 2,2% · `serializedCatalog` 1,1%**. Băm revision **chiếu lại từng bản ghi** mỗi lần tính (BTI 200k+ quote → hàng trăm nghìn object mỗi vòng, kéo GC). Cache theo định danh object **chỉ bật cho BTI/SBOBET** và adapter dựng object mới mỗi lần nên gần như không trúng. Sửa đúng = giữ định danh object cho hàng không đổi xuyên các lần dựng catalog — **việc thật, chưa làm** | `CPU.*.cpuprofile` |
| — | **Tôi đã quy sai và ship nhầm:** tưởng bộ ghi đĩa là thủ phạm CPU (thấy `catalog-cache` ghi 356 MB/phút) nên thêm chu-kỳ-mỗi-sàn cho `LatestCatalogPersister`. Profile cho thấy `serializedCatalog` chỉ **1,1%**. Bản vá tự nó vô hại (4 test xanh) nhưng **không sửa thứ nó nói**. Số lần ghi gần như không đổi (6→5/phút). |
| 47 | **Tab bị thay không chiếm lại được quyền — ĐÃ VÁ** | Gốc: `#acceptSource` **khoá vào nguồn đầu tiên nhìn thấy rồi từ chối mọi nguồn khác vĩnh viễn**; đường gỡ khoá chỉ chạy khi có `INVALIDATE` rõ ràng, mà tab biến mất lặng lẽ **không để lại dấu** — nên `sourceId` tab mới không khớp và **mọi `BASELINE` nó gửi đều bị vứt**, trong khi `activeGeneration` vẫn trỏ tab đã chết. Đo: APSPORT nằm **80 phút** với tab mới đã gắn và đang gửi, chỉ `TAB_STATE` đi qua; không đường khôi phục nào phá được (reload `BROWSER_REFRESH_DISABLED`, `navigate-lobby` đòi source đã gắn). **Vá:** nhận `BASELINE` từ nguồn khác **chỉ khi chủ cũ đã quá hạn baseline của chính nó**, epoch đã nghỉ vẫn từ chối. Hai test ghim cả hai chiều: trong hạn **từ chối**, quá hạn **nhận** | `provider-feed-controller.test.ts` 66 xanh |
...
| IM | **Anh bảo bỏ.** IM chết thì **KHÔNG phải lỗi, không báo, không sửa, không tính vào "sàn không LIVE"**. Bảng đủ sàn là **5**: CMD · SABA · SBOBET · APSPORT · BTI |
...
- **"SABA không có chuỗi census" (mục 44) — SAI, đã xoá.** Census có sẵn và đã ra
  `/api/diag/pipeline` từ trước: `sabaCollector` trong `wsAttach`, đủ `owners`, `captures`,
  `collectorState`, `lastErrorCode`. Tôi chỉ grep `catalogShape`, không thấy, rồi kết luận
  là **không có** — đúng lỗi đọc một **chỗ trống** thành một **sự thật**, lần thứ năm.
- **Kèo ẩn SABA = 0 thì đã đóng từ trước**, nằm ở mục TRẦN NHÀ CÁI (*More mở ra rỗng 52/52*).
  Census hôm nay xác nhận lại độc lập: `captures=o12.n12` — mở 12, rỗng 12. Mục 43/44 tôi
  viết ra đã **đào lại thứ ghi rõ là đừng đào**.
- **`domBlocked: true` KHÔNG phải triệu chứng.** Nó là cổng chặn xuất bản DOM **trong lúc**
  collector quét (`collectorState: RUNNING`), xoá ngay sau đó. Suýt báo thành lỗi.
