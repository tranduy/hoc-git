...
const outputPath = process.argv[3] ?? "bti-ui-runtime-evidence.json";
const startedAtMs = Date.now();
const browser = await chromium.launch({ headless: true });
const page = await browser.newPage();
const prices = new Map();
const pendingResponses = new Set();
const result = { startedAtMs, durationMs, documentNavigations: 0, btiCatalogResponses: 0,
  btiQuoteChanges: [], pageErrors: [] };
...
page.on("response", (response) => {
  if (!response.url().includes("/api/catalog/accounts/catalog-source%3ABTI%3AFOOTBALL") &&
    !response.url().includes("/api/catalog/accounts/catalog-source:BTI:FOOTBALL")) return;
  if (!response.ok()) return;
  const task = (async () => {
    try {
      const catalog = await response.json();
      result.btiCatalogResponses += 1;
      for (const quote of catalog.quotes ?? []) {
        ...
        if (prior !== undefined && prior.rawOdds !== quote.rawOdds && result.btiQuoteChanges.length < 50) {
          result.btiQuoteChanges.push({ key, before: prior.rawOdds, after: quote.rawOdds,
