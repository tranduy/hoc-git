...
const outputPath = process.argv[3] ?? "sbobet-ui-runtime-evidence.json";
const startedAtMs = Date.now();
const browser = await chromium.launch({ headless: true });
const page = await browser.newPage();
const prices = new Map();
const result = { startedAtMs, durationMs, documentNavigations: 0, sbobetCatalogResponses: 0,
  sbobetQuoteChanges: [], pageErrors: [] };
page.on("request", (request) => { if (request.resourceType() === "document") result.documentNavigations += 1; });
page.on("pageerror", (error) => result.pageErrors.push(error.message));
page.on("response", async (response) => {
  if (!response.url().includes("/api/catalog/accounts/catalog-source%3ASBOBET%3AFOOTBALL") &&
    !response.url().includes("/api/catalog/accounts/catalog-source:SBOBET:FOOTBALL")) return;
  if (!response.ok()) return;
  try {
    const catalog = await response.json();
    result.sbobetCatalogResponses += 1;
    for (const quote of catalog.quotes ?? []) {
      ...
      if (prior !== undefined && prior.rawOdds !== quote.rawOdds && result.sbobetQuoteChanges.length < 50) {
        result.sbobetQuoteChanges.push({ key, before: prior.rawOdds, after: quote.rawOdds,
