...
const outputPath = process.argv[2] ?? "sbobet-direct-price-evidence.json";
...
for (const marketType of ["FT_AH", "FT_TOTAL"]) {
  ...
  for (const displayed of candidates.slice(0, maxCandidates)) {
    ...
    if (!event) continue;
    const leg = { provider: "SBOBET", accountId, providerEventId: displayed.providerEventId,
  ...
  if (!accepted) throw new Error(`SBOBET_${marketType}_CHECK_UNAVAILABLE`);
