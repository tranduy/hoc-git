...
const outputPath = process.argv[3] ?? "im-runtime-evidence.json";
...
const result = { startedAtMs, durationMs, samples: 0, liveSamples: 0, zeroEventWhileLive: 0,
  ...
  catalogRevisionChanges: 0, websocketImRevisions: 0, websocketErrors: [], quoteChanges: [] };
const prices = new Map();
const socket = new WebSocket(baseUrl.replace(/^http/u, "ws") + "/api/realtime");
socket.addEventListener("message", (event) => {
  try {
    const message = JSON.parse(String(event.data));
    if (message.type === "CATALOG_REVISION" && message.accountId === accountId) result.websocketImRevisions += 1;
...
while (Date.now() < startedAtMs + durationMs) {
  try {
    ...
    const source = sources.sources.find((candidate) => candidate.lobby === "IM");
