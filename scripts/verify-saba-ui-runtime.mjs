...
const outputPath = process.argv[3] ?? "saba-ui-runtime-evidence.json";
const startedAtMs = Date.now();
const browser = await chromium.launch({ headless: true });
const page = await browser.newPage();
const prices = new Map();
const result = { startedAtMs, durationMs, documentNavigations: 0, sabaCatalogResponses: 0,
  sabaQuoteChanges: [], pageErrors: [] };
...
page.on("response", async (response) => {
  if (!response.url().includes("catalog-source%3ASABA%3AFOOTBALL") &&
    !response.url().includes("catalog-source:SABA:FOOTBALL")) return;
  if (!response.ok()) return;
  try {
    const catalog = await response.json();
    result.sabaCatalogResponses += 1;
    for (const quote of catalog.quotes ?? []) {
      ...
      if (prior !== undefined && prior.rawOdds !== quote.rawOdds && result.sabaQuoteChanges.length < 50) {
        result.sabaQuoteChanges.push({ key, before: prior.rawOdds, after: quote.rawOdds,
