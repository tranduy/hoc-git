...
const outputPath = process.argv[3] ?? "im-ui-runtime-evidence.json";
const startedAtMs = Date.now();
const browser = await chromium.launch({ headless: true });
const page = await browser.newPage();
const prices = new Map();
const result = { startedAtMs, durationMs, documentNavigations: 0, imCatalogResponses: 0,
  imQuoteChanges: [], pageErrors: [] };
page.on("request", (request) => { if (request.resourceType() === "document") result.documentNavigations += 1; });
page.on("pageerror", (error) => result.pageErrors.push(error.message));
page.on("response", async (response) => {
  if (!response.url().includes("/api/catalog/accounts/catalog-source%3AIM%3AFOOTBALL") &&
    !response.url().includes("/api/catalog/accounts/catalog-source:IM:FOOTBALL")) return;
  if (!response.ok()) return;
  try {
    const catalog = await response.json();
    result.imCatalogResponses += 1;
    for (const quote of catalog.quotes ?? []) {
      ...
      if (prior !== undefined && prior.rawOdds !== quote.rawOdds && result.imQuoteChanges.length < 50) {
        result.imQuoteChanges.push({ key, before: prior.rawOdds, after: quote.rawOdds,
