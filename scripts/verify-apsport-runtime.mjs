...
const outputPath = process.argv[3] ?? "apsport-runtime-evidence.json";
...
const result = { startedAtMs, durationMs, samples: 0, liveSamples: 0, zeroEventWhileLive: 0,
  ...
  catalogRevisionChanges: 0, websocketApsportRevisions: 0, websocketErrors: [], quoteChanges: [] };
...
socket.addEventListener("message", (event) => {
  try {
    ...
    if (message.type === "CATALOG_REVISION" && message.accountId === accountId) {
      result.websocketApsportRevisions += 1;
    }
...
while (Date.now() < deadlineMs) {
  try {
    ...
    const source = sources.sources.find((candidate) => candidate.lobby === "TSPORT");
